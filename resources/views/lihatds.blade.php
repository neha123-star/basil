...
@section('content')
			...
			<div class="container">
				...
				@foreach($lihatds as $ds)
				<div class="row mb-4">
						<div class="col-md-4">
							<img src="{{url('assets/images/desain/'.$ds->gambar_ds)}}" alt="..." class="img-thumbnail">
						</div>
						<div class="col-md-8">
							<div class="text">
								<h5>{{$ds->judul_ds}}</h5>
								<h5>Rp <label class="uang"> {{$ds->harga_ds}}</label></h5>
								<p class="text-truncate">{{$ds->isi_ds}}</p>
								<a href="/tampil/editds/{{ $ds->id }}" class="btn btn-outline-dark">Edit</a>
								<a href="/tampil/hapusds/{{ $ds->id}}" class="btn btn-outline-dark">Hapus</a>
							</div>
						</div>
				</div>
				@endforeach
