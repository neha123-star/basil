...
@section('content')
			...
			<div class="container">
				...
				@foreach($lihatpg as $pg)
				<div class="row mb-4">
						<div class="col-md-4">
							<img src="{{url('assets/images/pg/'.$pg->gambar_pg)}}" alt="..." class="img-thumbnail">
						</div>
						<div class="col-md-8">
							<div class="text">
								<h5>{{$pg->judul_pg}}</h5>
								<h5>Rp <label class="uang"> {{$pg->harga_pg}}</label></h5>
								<p class="text-truncate">{{$pg->isi_pg}}</p>
								<a href="/tampil/editpg/{{ $pg->id }}" class="btn btn-outline-dark">Edit</a>
								<a href="/tampil/hapuspg/{{ $pg->id}}" class="btn btn-outline-dark">Hapus</a>
							</div>
						</div>
				</div>
				@endforeach
